...
def validarModifFechaInicio(fechainicio,fechafin):
    try:
        ...
        fecha_actual=date.today()
        if fecha_ing>=fecha_actual and fecha_ing<=fecha_fin:  ##aca verifico ademas que la fecha de inicio sea menor que la de fin
             return True
        else:
...
def validarFechaFin(fechainicio,fechafin):
    try:
        fecha_ing=datetime.strptime(fechafin,"%d-%m-%Y").date()
        fechainicio=datetime.strptime(fechainicio,"%d-%m-%Y").date()
        if fecha_ing>=fechainicio:
             return True
        else:
